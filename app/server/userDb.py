...
class User(object):
    ...
    def getUserSubInfo(self,param):
        sql = " SELECT s.name FROM user_sub as us \
                INNER JOIN subscibe as s \
                ON us.subscibe_id = s.id \
                WHERE us.uid = %s and us.status=1 "
        return self.db.exeAllMySQL(sql,param)
    
    def getSubInfo(self,param):
        sql = " SELECT * FROM subscibe WHERE name = %s or id = %s "
        return self.db.exeOneMySQL(sql,param)
    
    def InsertUserSubInfo(self,param):
        sql = " INSERT INTO user_sub (uid,subscibe_id,status) values(%s,%s,%s) "
        self.db.insertMySQL(sql,param)
    
    def updateUserSubInfo(self,param):
        sql = " UPDATE user_sub SET status = 0 WHERE uid = %s and subscibe_id = %s "
        self.db.updateMySQL(sql,param)
    
    def isSubUserInfo(self,param):
        sql = " SELECT * FROM user_sub WHERE uid = %s and subscibe_id = %s "
        return self.db.exeOneMySQL(sql,param)    
